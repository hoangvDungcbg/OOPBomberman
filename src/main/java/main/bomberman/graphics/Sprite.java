...
public class Sprite {
    ...
    */
    ...
    public static Image removebg(Image img, int scalefc){
        int W = (int) img.getWidth();
        int H = (int) img.getHeight();
        WritableImage outputImage = new WritableImage(W*scalefc, H*scalefc);
        PixelReader reader = img.getPixelReader();
        PixelWriter writer = outputImage.getPixelWriter();
        Color oldColor = img.getPixelReader().getColor(W - 1,H - 1);
        ...
        */
        for (int y = 0; y < H; y++) {
            for (int x = 0; x < W; x++) {
                ...
                for(int e = 0; e < scalefc; e++)
                    for (int f = 0; f < scalefc; f++)
                        writer.setArgb(x*scalefc + e, y*scalefc + f, argb);
            }
        }
        return outputImage;
    }
    ...
    public static Image[] getListImage(String name, int number, int scale) {
        ...
        for (int i = 0; i < number; i++) {
            imageArray[i] = new Image(Objects.requireNonNull(SetImage.set(name + i + ".png")));
            imageArray[i] = removebg(imageArray[i], scale);
        }
        return imageArray;
    }
